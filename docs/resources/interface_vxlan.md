...
- `bridge` (String) The bridge interface the VXLAN interface is dynamically added to as a bridge port.
...
- `hw` (Boolean) Enables hardware offloading of the VXLAN interface on switches whose switch chip supports VXLAN tunnel termination. The attribute is only present on such hardware and the device enables it by default, therefore a value that the user did not explicitly set must not produce a diff.
