# routeros_routing_igmp_proxy_mfc (Data Source)


## Example Usage
```terraform
# Entries in the menu, narrowed by any attribute it returns.
# Omit `filter` to read all of them.
data "routeros_routing_igmp_proxy_mfc" "example" {
  filter = {
    disabled = "false"
  }
}

output "routing_igmp_proxy_mfc" {
  value = data.routeros_routing_igmp_proxy_mfc.example.entries[*].id
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Optional

- `___id___` (Number) <em>Resource ID type (.id / name). This is an internal service field, setting a value is not required.</em>
- `___path___` (String) <em>Resource path for CRUD operations. This is an internal service field, setting a value is not required.</em>
- `___skip___` (String) <em>A set of transformations for field names. This is an internal service field, setting a value is not required.</em>
- `filter` (Map of String) Additional request filtering options.

### Read-Only

- `entries` (List of Object) Entries read from `/routing/igmp-proxy/mfc`, narrowed by `filter`. (see [below for nested schema](#nestedatt--entries))
- `id` (String) The ID of this resource.

<a id="nestedatt--entries"></a>
### Nested Schema for `entries`

Read-Only:

- `active` (Boolean)
- `active_downstream_interfaces` (Set of String)
- `disabled` (Boolean)
- `downstream_interfaces` (Set of String)
- `dynamic` (Boolean)
- `group` (String)
- `id` (String)
- `source` (String)
- `upstream_interface` (String)


