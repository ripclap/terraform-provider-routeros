# routeros_wifi_datapath (Data Source)


## Example Usage
```terraform
# Entries in the menu, narrowed by any attribute it returns.
# Omit `filter` to read all of them.
data "routeros_wifi_datapath" "example" {
  filter = {
    name = "example"
  }
}

output "wifi_datapath" {
  value = data.routeros_wifi_datapath.example.entries[*].name
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Optional

- `___id___` (Number) <em>Resource ID type (.id / name). This is an internal service field, setting a value is not required.</em>
- `___path___` (String) <em>Resource path for CRUD operations. This is an internal service field, setting a value is not required.</em>
- `filter` (Map of String) Additional request filtering options.

### Read-Only

- `entries` (List of Object) Entries read from `/interface/wifi/datapath`, narrowed by `filter`. (see [below for nested schema](#nestedatt--entries))
- `id` (String) The ID of this resource.

<a id="nestedatt--entries"></a>
### Nested Schema for `entries`

Read-Only:

- `bridge` (String)
- `bridge_cost` (String)
- `bridge_horizon` (String)
- `client_isolation` (Boolean)
- `comment` (String)
- `disabled` (Boolean)
- `id` (String)
- `interface_list` (String)
- `name` (String)
- `traffic_processing` (String)
- `vlan_id` (Number)


