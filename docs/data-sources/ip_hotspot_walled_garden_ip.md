# routeros_ip_hotspot_walled_garden_ip (Data Source)


## Example Usage
```terraform
# Entries in the menu, narrowed by any attribute it returns.
# Omit `filter` to read all of them.
data "routeros_ip_hotspot_walled_garden_ip" "example" {
  filter = {
    comment = "managed by terraform"
  }
}

output "ip_hotspot_walled_garden_ip" {
  value = data.routeros_ip_hotspot_walled_garden_ip.example.entries[*].id
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Optional

- `___id___` (Number) <em>Resource ID type (.id / name). This is an internal service field, setting a value is not required.</em>
- `___path___` (String) <em>Resource path for CRUD operations. This is an internal service field, setting a value is not required.</em>
- `filter` (Map of String) Additional request filtering options.

### Read-Only

- `entries` (List of Object) Entries read from `/ip/hotspot/walled-garden/ip`, narrowed by `filter`. (see [below for nested schema](#nestedatt--entries))
- `id` (String) The ID of this resource.

<a id="nestedatt--entries"></a>
### Nested Schema for `entries`

Read-Only:

- `action` (String)
- `comment` (String)
- `disabled` (Boolean)
- `dst_address` (String)
- `dst_address_list` (String)
- `dst_host` (String)
- `dst_port` (String)
- `id` (String)
- `invalid` (Boolean)
- `protocol` (String)
- `server` (String)
- `src_address` (String)
- `src_address_list` (String)


