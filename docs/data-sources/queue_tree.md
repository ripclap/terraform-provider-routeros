# routeros_queue_tree (Data Source)


## Example Usage
```terraform
# Entries in the menu, narrowed by any attribute it returns.
# Omit `filter` to read all of them.
data "routeros_queue_tree" "example" {
  filter = {
    name = "example"
  }
}

output "queue_tree" {
  value = data.routeros_queue_tree.example.entries[*].name
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Optional

- `___id___` (Number) <em>Resource ID type (.id / name). This is an internal service field, setting a value is not required.</em>
- `___path___` (String) <em>Resource path for CRUD operations. This is an internal service field, setting a value is not required.</em>
- `___skip___` (String) <em>A set of transformations for field names. This is an internal service field, setting a value is not required.</em>
- `filter` (Map of String) Additional request filtering options.

### Read-Only

- `entries` (List of Object) Entries read from `/queue/tree`, narrowed by `filter`. (see [below for nested schema](#nestedatt--entries))
- `id` (String) The ID of this resource.

<a id="nestedatt--entries"></a>
### Nested Schema for `entries`

Read-Only:

- `bucket_size` (String)
- `burst_limit` (String)
- `burst_threshold` (String)
- `burst_time` (String)
- `comment` (String)
- `disabled` (Boolean)
- `dynamic` (Boolean)
- `id` (String)
- `invalid` (Boolean)
- `limit_at` (String)
- `max_limit` (String)
- `name` (String)
- `packet_mark` (Set of String)
- `parent` (String)
- `priority` (Number)
- `queue` (String)


